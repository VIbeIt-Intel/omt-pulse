...
export function RadioPttButton({
  ...
  className?: string;
  label?: string;
  /** Inline slim control for collapsed dock. */
  compact?: boolean;
}) {
  ...
  return (
    <button
      ...
      aria-pressed={active}
      className={cn(
        "select-none touch-manipulation font-bold transition-colors",
        "[-webkit-user-select:none] [-webkit-touch-callout:none]",
        compact
          ? "rounded-xl px-3 py-2 text-xs flex flex-row items-center justify-center gap-1.5 min-h-9 w-auto"
          : "rounded-2xl px-6 py-5 text-base flex flex-col items-center justify-center gap-1.5 min-h-[5.5rem] w-full",
        active
    ...
    >
      <Radio className={cn(compact ? "h-4 w-4" : "h-7 w-7", active && "animate-pulse")} />
      <span>
        {transmitting
          ? compact
            ? "On air"
