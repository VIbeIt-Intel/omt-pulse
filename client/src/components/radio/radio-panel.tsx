...
export function RadioPanel({
  ...
  if (dock && collapsed) {
    const collapsedStatus = radio.transmitting
        ...
        : radio.connecting || channelsLoading || (enabled && commandId == null)
          ...
          : radio.connected
            ? radio.speakerReady
              ...
              : { label: "Muted", tone: "wait" as const }
            : radio.error
              ? { label: "Failed", tone: "bad" as const }
              : { label: "Offline", tone: "bad" as const };

    const toneClass =
      collapsedStatus.tone === "live"
        ? "border-emerald-500/40 bg-emerald-950/30"
        : collapsedStatus.tone === "talk"
          ? "border-amber-500/40 bg-amber-950/25"
          : collapsedStatus.tone === "wait"
    ...
    const dotClass =
      collapsedStatus.tone === "live"
        ? "bg-emerald-400"
        : collapsedStatus.tone === "talk" || collapsedStatus.tone === "wait"
          ? "bg-amber-400"
          : "bg-red-400";
    ...
    return (
      <div
        ...
        data-collapsed="true"
        data-radio-status={collapsedStatus.label.toLowerCase()}
      >
        <div className="flex items-center gap-2">
          <button
            type="button"
            className="shrink-0 rounded-md p-1.5 text-emerald-400 hover:bg-emerald-500/10"
          ...
          <div className="min-w-0 flex-1">
              ...
              >
                <span
                  className={cn(
                    ...
                    dotClass,
                    (collapsedStatus.tone === "live" || collapsedStatus.tone === "talk") &&
                      "animate-pulse",
                  )}
                  aria-hidden
                />
            ...
            <p className="truncate text-[10px] text-muted-foreground leading-tight">
              {radio.remoteTalking || radio.transmitting
                ? statusLine
                : radio.error
                  ? radio.error
                  : channelLabel}
