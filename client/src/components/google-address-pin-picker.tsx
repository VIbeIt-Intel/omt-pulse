...
type Props = {
  value: AddressPinValue;
  onChange: (value: AddressPinValue) => void;
  disabled?: boolean;
};

/** Google Places search + map pin for predefined premises / locations. */
export function GoogleAddressPinPicker({ value, onChange, disabled }: Props) {
  ...
  const [mapDraft, setMapDraft] = useState<AddressPinValue | null>(null);
  ...
  function openMapDialog() {
    ...
    setMapDraft(
      hasCoords
        ? { ...value }
        : { address: value.address, latitude: null, longitude: null },
    );
    setMapOpen(true);
  }
    ...
    const timer = window.setTimeout(() => {
      ...
      map.addListener("click", (e: google.maps.MapMouseEvent) => {
        ...
        if (markerRef.current) {
          ...
          markerRef.current = new google.maps.Marker({ position, map, title: "Location" });
        }
        ...
        geocoderRef.current.geocode({ location: position }, (results, status) => {
          ...
          if (markerRef.current) markerRef.current.setTitle(address);
        });
      });
    }, 50);
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-2">
        <Label htmlFor="location-address-search">
          Address <span className="text-muted-foreground text-xs font-normal">(Google search)</span>
        </Label>
        <Button
          type="button"
          variant="outline"
        ...
        >
          ...
          {value.latitude != null && value.longitude != null ? "Adjust pin" : "Pick on map"}
        </Button>
      </div>
      ...
      <div className="relative">
        <Input
          ...
          value={search}
          disabled={disabled || !mapsReady}
          placeholder="Start typing a street, estate, or business…"
          onChange={(e) => {
      ...
      {value.latitude != null && value.longitude != null ? (
        <div className="flex items-center justify-between rounded-md border px-3 py-2 bg-muted/40 gap-2">
        ...
        <p className="text-xs text-muted-foreground italic">
          Pick a Google suggestion or drop a pin on the map
        </p>
      )}
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Click the map to place the pin. The address fills in from Google.
            </p>
            <div
              ref={mapRef}
              className="h-[min(55vh,420px)] w-full rounded-md border bg-muted/30"
              data-testid="location-google-map-picker"
            />
            <div className="rounded-md border bg-card/60 px-3 py-2 text-xs space-y-1 min-h-[3rem]">
              {mapGeocoding ? (
                ...
                <>
                  ...
                  <p className="text-muted-foreground tabular-nums" data-testid="text-map-coordinates">
                    {mapDraft.latitude.toFixed(6)}, {mapDraft.longitude.toFixed(6)}
                  </p>
                </>
              ) : (
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={closeMapDialog} data-testid="button-cancel-map">
              Cancel
            </Button>
            <Button
              type="button"
