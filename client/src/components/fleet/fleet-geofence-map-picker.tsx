...
export function FleetGeofenceMapPicker({
  ...
  useEffect(() => {
    ...
    // Container may finish layout after first paint — force a proper tile fill.
  ...
  return (
    <div className={cn("space-y-4", className)} data-testid="fleet-geofence-map-picker">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start">
            ...
            <div className="absolute z-20 mt-1 w-full rounded-lg border bg-popover shadow-lg max-h-48 overflow-y-auto">
              {suggestions.map((s) => (
                <button
                  key={s.place_id}
                  type="button"
                  className="w-full text-left px-3 py-2.5 text-sm hover:bg-muted/60 border-b last:border-0"
        ...
        {vehicleLatLng && (
          <Button type="button" variant="outline" className="h-11 shrink-0" onClick={centreOnVehicle}>
      ...
      <div className="relative rounded-xl border border-border/70 overflow-hidden bg-slate-950 shadow-inner min-h-[320px]">
        {!mapsReady && (
          <div
        ...
        <div ref={mapRef} className="w-full min-h-[320px]" style={{ height }} />
      </div>
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">
              Zone size from the pin centre
            </p>
          </div>
          <div className="text-right">
            <p className="text-sm font-semibold tabular-nums">{value.radiusM.toLocaleString()} m</p>
            <p className="text-[11px] text-muted-foreground tabular-nums">{radiusKm} km</p>
          </div>
        </div>
        <Slider
          min={MIN_RADIUS_M}
        ...
        {hasCentre(value) && (
          <div className="grid grid-cols-2 gap-3 pt-1">
            <div className="rounded-lg border border-border/60 bg-muted/30 px-3 py-2">
              <p className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold">Lat</p>
              <p className="text-xs font-mono tabular-nums mt-0.5">{value.lat.toFixed(6)}</p>
            </div>
            <div className="rounded-lg border border-border/60 bg-muted/30 px-3 py-2">
              <p className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold">Lng</p>
              <p className="text-xs font-mono tabular-nums mt-0.5">{value.lng.toFixed(6)}</p>
            </div>
          </div>
        )}
      </div>
